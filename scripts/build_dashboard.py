...
# 🗞️ News-tab feed (dated bullish stock-referencing news + new candidates); optional.
# Generated by scripts/build_news_feed.py (run before this script in /update-all).
NEWS_FEED = os.path.join(ROOT, "dashboard", "data", "news_feed.json")
...
def load_news_feed(path):
    """News feed JSON (dated recaps + by-ticker bullish news), or {} if unavailable."""
    try:
        with open(path, encoding="utf-8") as f:
            return json.load(f)
    except Exception:
...
# ---------- News tab (dated bullish stock-referencing news + candidates) -----

def _news_prose_html(md):
    """Light render of an agent-written prose recap: escape, linkify [t](url),
    blank-line-separated blocks -> paragraphs."""
    out = []
    for block in re.split(r"\n\s*\n", md.strip()):
        s = html.escape(block.strip())
        s = re.sub(r"\[([^\]]+)\]\((https?://[^)\s]+)\)",
...
def _news_tk_badges(tickers, pages, cands=None):
    """Ticker badges; tracked names deep-link to their latest decision page,
    untracked $cashtags render as muted 'new' chips."""
    bits = []
    for tk in tickers:
        page = pages.get(tk)
        lbl = html.escape(tk)
        bits.append(f'<a class="badge v-aligned" href="{page}" title="open {lbl} decision page">{lbl}</a>'
                    if page else f'<span class="badge muted">{lbl}</span>')
    for c in (cands or []):
        cc = html.escape(c)
        bits.append(f'<a class="badge b-neu" href="https://x.com/search?q=%24{cc}&f=live" '
                    f'target="_blank" rel="noopener" title="not yet tracked — live X chatter for ${cc}">${cc}<sup>new</sup></a>')
    return " ".join(bits)


def build_news(feed, pages):
    """Render the 🗞️ News tab: dated human-readable recaps (optional agent prose +
    deterministic auto-digest), bullish stock-referencing headlines, inferred new
    candidates, and the primary-source industry desk. `pages` maps ticker -> latest
    decision-page filename (or None)."""
    if not feed or not (feed.get("dates") or feed.get("by_ticker")):
        return ('<p class="muted">No news feed yet. Generate it with '
                '<code>python3 scripts/build_news_feed.py</code> (or run <code>/update-all</code>), '
                'then rebuild this dashboard.</p>')
    meta = (f'<p class="sub" style="margin:-4px 0 14px">Last {feed.get("window_days","?")} days · '
            f'{feed.get("n_bullish_items",0)} bullish stock-referencing items from '
            f'{feed.get("n_scanned",0)} scanned (keyless X Brain news lane + FinTwit) + the '
            f'primary-source industry desk (ai-news). Generated {html.escape(str(feed.get("generated","")))}. '
            f'Coarse lexicon signal — <b>not</b> advice.</p>')

    # ---- dated recap cards (newest first) ----
    cards = []
    for d in feed.get("dates", []):
        head = (f'<div style="display:flex;align-items:baseline;gap:10px;margin-bottom:4px">'
                f'<h3 style="margin:0">{html.escape(d["date"])}</h3>'
                f'<span class="muted">{d.get("n_items",0)} bullish item(s)</span></div>')
        prose = (f'<div class="newsprose" style="border-left:3px solid #2ea043;padding:2px 0 2px 12px;margin:6px 0">'
                 f'{_news_prose_html(d["prose"])}</div>') if d.get("prose") else ''
        digest = f'<p class="muted" style="margin:4px 0 10px">{html.escape(d.get("digest",""))}</p>'
        # bullish headlines that day
        lis = ''
        for it in d.get("items", [])[:14]:
            t = html.escape(it["title"]) or "(untitled)"
            link = (f'<a href="{html.escape(it["url"])}" target="_blank" rel="noopener">{t}</a>'
                    if it.get("url") else t)
            badges = _news_tk_badges(it.get("tickers", []), pages, it.get("cands"))
            lis += (f'<li style="margin:3px 0">{badges} '
                    f'<span class="b-pos" title="net bullish lexicon score">+{it["net"]}</span> {link}</li>')
        items_html = f'<ul style="list-style:none;padding:0;margin:0 0 8px">{lis}</ul>' if lis else ''
        # industry desk
        ind = ''
        if d.get("industry"):
            irows = ''.join(
                f'<li style="margin:2px 0"><a href="{html.escape(a["url"])}" target="_blank" rel="noopener">'
                f'{html.escape(a["title"])}</a> <span class="muted">· {html.escape(a["source"])}</span></li>'
                for a in d["industry"][:8])
            ind = (f'<details style="margin:4px 0 2px"><summary class="muted" style="cursor:pointer">'
                   f'🏭 Industry desk · {len(d["industry"])} primary-source article(s)</summary>'
                   f'<ul style="list-style:none;padding:6px 0 0;margin:0">{irows}</ul></details>')
        cards.append(f'<div class="card" style="text-align:left;display:block;margin:0 0 14px;padding:14px 16px">'
                     f'{head}{prose}{digest}{items_html}{ind}</div>')
    recap_html = "".join(cards) or '<p class="muted">No dated news in the window yet.</p>'

    # ---- bullish stocks in the news (tracked names) ----
    brows = ''
    for b in feed.get("by_ticker", []):
        tk = b["ticker"]; page = pages.get(tk)
        tkcell = (f'<a href="{page}"><b>{html.escape(tk)}</b></a>' if page else f'<b>{html.escape(tk)}</b>')
        flag = ('<span class="badge b-pos" title="already has a /trading-analysis decision">analyzed</span>'
                if b.get("analyzed") else
                '<span class="badge b-neu" title="in the tracked universe but no decision on file yet">untracked</span>')
        top = b["headlines"][0] if b.get("headlines") else None
        topcell = (f'<a href="{html.escape(top["url"])}" target="_blank" rel="noopener">{html.escape(top["title"][:90])}</a>'
                   f' <span class="muted">· {html.escape(top["date"])}</span>') if top else '<span class="muted">—</span>'
        brows += (f'<tr><td>{tkcell}</td><td>{flag}</td>'
                  f'<td class="num b-pos" data-s="{b["net"]}">+{b["net"]}</td>'
                  f'<td class="num" data-s="{b["mentions"]}">{b["mentions"]}</td>'
                  f'<td>{topcell}</td></tr>')
    bull_html = (
        '<table id="newsbull"><thead><tr>'
        '<th onclick="sortBy(0,this)" title="Stock symbol — click to open its decision page.">Ticker</th>'
        '<th onclick="sortBy(1,this)" title="Whether this name already has a /trading-analysis decision on file.">Status</th>'
        '<th onclick="sortBy(2,this,1)" title="Sum of net bullish lexicon score across this name\'s news items in the window.">Bull score</th>'
        '<th onclick="sortBy(3,this,1)" title="How many bullish news items referenced this name in the window.">Items</th>'
        '<th title="Most recent / strongest bullish headline for this name.">Top headline</th>'
        f'</tr></thead><tbody>{brows}</tbody></table>') if brows else \
        '<p class="muted">No tracked names in bullish news this window.</p>'

    # ---- new candidates (untracked $cashtags surfacing in bullish news) ----
    crows = ''
    for c in feed.get("new_candidates", []):
        cc = html.escape(c["ticker"])
        s = c.get("sample") or {}
        samp = (f'<a href="{html.escape(s.get("url",""))}" target="_blank" rel="noopener">{html.escape(s.get("title","")[:90])}</a>'
                if s.get("url") else html.escape(s.get("title", "")))
        crows += (f'<li style="margin:4px 0"><a class="badge b-neu" href="https://x.com/search?q=%24{cc}&f=live" '
                  f'target="_blank" rel="noopener" title="live X chatter for ${cc}">${cc}</a> '
                  f'<span class="muted">×{c["mentions"]} · {html.escape(c.get("last_seen",""))}</span> — {samp}</li>')
    cand_html = (f'<ul style="list-style:none;padding:0;margin:0">{crows}</ul>'
                 if crows else '<p class="muted">No new (untracked) names surfaced in bullish news this window. '
                 'Tickers appearing here would be candidates for a fresh <code>/trading-analysis</code>.</p>')

    return (
        f'{meta}'
        f'<h2>📅 Daily recap <span class="muted" style="text-transform:none;font-weight:400">'
        f'· newest first · agent prose (when written) over a deterministic auto-digest</span></h2>'
        f'{recap_html}'
        f'<h2>🐂 Bullish stocks in the news <span class="muted" style="text-transform:none;font-weight:400">'
        f'· tracked names referenced in bullish items · click a ticker for its decision page, a header to sort</span></h2>'
        f'{bull_html}'
        f'<h2>🆕 New on the radar <span class="muted" style="text-transform:none;font-weight:400">'
        f'· $cashtags NOT yet tracked, surfacing in bullish news — inferred beneficiaries to consider analyzing</span></h2>'
        f'{cand_html}')
...
def main():
    ...
    # ---- News tab (dated bullish stock-referencing news) ----
    ticker_pages = {r["ticker"]: r.get("page") for r in latest.values()}
    news_html = build_news(load_news_feed(NEWS_FEED), ticker_pages)
