...
2.5. **News recap (prose)** — After the macro cycle, the AI (you) should write a short
   plain-English recap of today's notable **bullish news referencing specific stocks** into
   `news-recaps/<DATE>.md` (repo-relative; 2–4 sentences — what moved, which names, why it
   matters; draw on the freshly-refreshed X Brain news + industry desk and the cycle read).
   This is **optional but recommended**: the News tab always shows a deterministic
   auto-digest, and renders your prose on top of it when this file exists. Skippable if the
   tape is quiet.
3. **Dashboard** — `scripts/build_news_feed.py` then `scripts/build_dashboard.py`: first
   distil the **🗞️ News-tab feed** (`dashboard/data/news_feed.json` — dated bullish
   stock-referencing news from the X Brain news lane + the ai-news industry desk, the
   per-ticker rollup, and inferred **new (untracked) candidates**), then regenerate
   `dashboard/` HTML (per-decision pages + index + best-call leaderboards + **🔔 Changes**,
   **🔬 Research**, and **🗞️ News** tabs) from the decision log + `analyzed-stocks/`.
...
- **News tab:** the news-feed line (bullish stock-ref items / scanned, tracked names, new
  candidates, recap days) + call out any **new (untracked) candidates** that surfaced in
  bullish news (worth a fresh `/trading-analysis`).
